...
- **FX chain**: eq → distortion → wah → phaser → delay → reverb → duck (each independently bypassable)
- **Equalizer, one per machine**: a folded **EQUALIZER** section under the scope with a tab for the **Sequencer**, the **Drum Machine** and the **Sampler**, each holding its own eight-band EQ (off until you switch it on — the lamp in front of each tab tells you which lanes are working). **Draw the curve** straight onto the graph; double-tap a band to put it back, Shift-drag for fine moves. **HP** and **LP** are real filters rather than tilts, and **Q** sharpens every band from a broad tone shape to a narrow notch for one problem frequency. Switch on the **ⓘ** badges for what each band name (SUB, MUD, BOX, NASAL, PRES, HARSH, SIB, AIR) means. Presets cover the usual moves — Low Pass, High Pass, Band Pass, Telephone, Hiss Removal, Rumble Cut, Warmth, Presence, Air — plus one per problem band the Spectrum names: **De-Mud**, **De-Box**, **De-Nasal** and **De-Harsh**. The bands sit on the same frequencies and the same log scale the Spectrum draws, and the panel lines up under it, so a bump you can see there is under the band that fixes it here. Each EQ runs at the **front** of its lane, which is what lets a highpass on the drums stop the kick pumping the rest of the kit, and costs nothing at all while switched off
...
- **Sampler slots are full voices**: pick a slot and the strip under the grid becomes *that slot's* channel — **pitch** it ±2 octaves (varispeed, so it stretches as it drops), **trim** where it starts and ends, play it **backwards**, shape it with an **attack/decay** and a resonant **tone** filter, then **pan** and level it. Two slots put in the same **choke group** cut each other off — a closed hat killing an open one — and a **mono** slot cuts its own previous hit instead of stacking. Every control is a no-op where it sits by default, so nothing you already made changes
- **Chop a break across the pads**: open a loop in the sample editor and cut it into 2–8 slices, either evenly or **at the transients** it finds for you, nudge any boundary by dragging it on the waveform, then **spread** the slices across the slots. Each one lands as an ordinary sample with its own channel, so you can pitch, reverse and filter the pieces independently and play the break back in any order from the grid. One press undoes the whole spread
- **Scratch a sample by drawing the gesture**: the sample editor's **Scratch** section puts the clip on a grid of sixteenths at your tempo and lets you draw the speed of the record over it — drag a point up and the needle runs faster and higher, pull it below the middle line and it runs backwards, and the lane above shows the result warping in real time so you can see it land on the beat before you hear it. The thin band on top is the **crossfader**: tap it to cut the sound out under a stroke, which is what turns a wobble into a scratch. Start from a classic (**Baby** is the short–short–long everyone knows, plus transformer, chirp, tear, flare, scribble and stab), or roll a random one. **Preview** hears it, **Scratch** prints it into the clip, and Undo takes it straight back. It is computed once on the button press, never on the audio thread, and the result is exactly one bar (or any count of sixteenths) long
- **Fit a loop to your tempo, without detuning it**: a break recorded at 96 BPM can be made to sit in a song at 128. **FIT** on the slot row retimes the clip to the nearest bar length in one press and offers **Undo**; the sample editor’s **Fit** row lets you name the target yourself — one bar, or any count of sixteenths, labelled against the song’s own meter so 3/4 and 7/8 read correctly — and preselects whichever is nearest the clip already, so the audio barely moves. Pick **Rhythmic** (transient-preserving, for drums and loops) or **Tonal** (smoother on pads and sustained sounds). The pitch does not move, which is what separates it from the slot’s varispeed PITCH; **Shift** beside it does the converse, moving the pitch and keeping the length. Both are hand-written — no dependency, and nothing extra on the audio thread, because the work happens once to the clip rather than on every hit
- **Transport**: clock, arpeggiator, **4-track** 16-step note sequencer (chords and counter-lines; tracks 2–4 need poly voicing, and fold away when unused, and each track has a **PAN** knob beside its mute so a bank spreads across the stereo field), 8-track drum machine, and 8-slot multi-track sampler
...
- **Pattern banks**: the sequencer, drum machine, sampler, and motion sequencer each start with 4 banks (A–D) and grow on demand to 8 (A–H) with the bank bar's + arm — per machine, independently copyable and chainable
- **Sampler sounds**: each of the 8 slots plays a one-shot sample. Load a WAV/MP3, or **record from your microphone and edit in-app** — crop, low/hi-pass, octave up/down, reverse, normalize, fade in/out, boost, fit to the bar, pitch-shift, scratch — then save WAV/MP3 or drop it straight into a slot; any loaded slot can be re-opened (✎) to edit again
...
- **Oscilloscope / spectrum display** (mono or stereo, with a max-dB peak-hold readout). The spectrum reads on a **log frequency scale** marked at 100 / 500 / 1k / 5k / 10k Hz — hover it to read the exact frequency under the pointer, or hit **Zones** to shade the four bands that usually need fixing (mud, boxy, nasal, harsh). Plus pitch-bend and mod wheels
...
- **Install it as an app**: add to your homescreen / install from the browser menu and VAST behaves like a native instrument — it **works offline**: press **Play offline** in Help & About to save every demo, dialog and help page on the device, and new versions update that copy by themselves, launches fullscreen on Android, **keeps the screen awake while the audio runs**, opens `.json`/`.websynth.zip` song files directly (desktop), stays audible with the iPhone mute switch on (iOS 17+), and there's a fullscreen toggle in the header for browser play too
...
npm test             # vitest run — unit tests (node; jsdom for tests/ui/**)
...
| --- | --- | --- | --- | --- |
| `websynth-song` | 8 | [song](public/schema/websynth-song.schema.json) | [song-mode](specs/features/song-mode.md) | The canonical song the app exports |
