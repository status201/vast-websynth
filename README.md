...
- **Equalizer, one per machine**: a folded **EQUALIZER** section under the scope with a tab for the **Sequencer**, the **Drum Machine** and the **Sampler**, each holding its own eight-band EQ (off until you switch it on — the lamp in front of each tab tells you which lanes are working). **Draw the curve** straight onto the graph; double-tap a band to put it back, Shift-drag for fine moves. **HP** and **LP** are real filters rather than tilts, and **WIDTH** takes every band from a broad tone shape to a narrow notch for one problem frequency. Presets cover the usual moves — Low Pass, High Pass, Band Pass, Telephone, Hiss Removal, Rumble Cut, Warmth, Presence, Air — plus one per problem band the Spectrum names: **De-Mud**, **De-Box**, **De-Nasal** and **De-Harsh**. The bands sit on the same frequencies and the same log scale the Spectrum draws, and the panel lines up under it, so a bump you can see there is under the band that fixes it here. Each EQ runs at the **front** of its lane, which is what lets a highpass on the drums stop the kick pumping the rest of the kit, and costs nothing at all while switched off
