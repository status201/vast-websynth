...
/**
 * How far an *early* nudge may reach, in seconds (step-settings.md REQ-an-early-offset-is-capped-in-seconds).
 *
 ...
 * the burst shape transport.md REQ-the-transport-catch-up-is-bounded exists to prevent. 60 ms keeps 15 ms of
 ...
 * (step-settings.md REQ-a-step-carries-a-micro-offset/REQ-micro-is-one-pure-offset). One definition, the way `Clock.swingOffset` is
 ...
 * fraction of what the user sees on the grid (meter.md REQ-each-machine-has-a-step-rate).
 *
...
export function forEachActiveHit(
  ...
  for (let lane = 0; lane < muted.length; lane++) {
    ...
    // the whole point of micro-timing (step-settings.md REQ-micro-is-one-pure-offset). Offsetting the
