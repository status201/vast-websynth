...
export interface LoopDriverDeps {
  ...
  songBars(): number;
  /** `Engine.canSeek` — a wrap is a seek, refused in the same states (REQ-a-loop-that-cannot-jump-does-not). */
  canSeek(): boolean;
  /** `Engine.seekTo` — the guarded entry point, for REQ-turning-loop-on-moves-the-cue's cue move. */
 ...
 *
 * - The router is installed **only while the loop is engaged** (REQ-loop-costs-nothing-unless-engaged), so an
 *   unlooped transport pays one null check per tick and a looping one does a
 *   modulo per tick and the real work once per bar.
 * - Engaging it while stopped cues the transport into the range (REQ-turning-loop-on-moves-the-cue).
 */
export class LoopDriver {
  ...
  private readonly route = (next: number): number => {
    if (next % this.barTicks !== 0) return next; // only a bar line decides (REQ-the-loop-wraps-only-on-a-bar-line)
    ...
    // an engaged loop instead of being bent by it (REQ-a-loop-that-cannot-jump-does-not).
  ...
  /** REQ-turning-loop-on-moves-the-cue: Play should start inside the loop — unless the cue already is. */
