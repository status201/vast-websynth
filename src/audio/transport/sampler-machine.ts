 ...
 * choice REQ-a-slot-starts-from-zero made for a choke that resolved to zero.
 */
 ...
 *
 * Each slot carries a channel (vol/pan/tone/res, REQ-each-slot-has-a-channel) and a per-hit voice
 * window (pitch/start/end/rev/attack/decay, REQ-a-hit-plays-a-window-of-the-buffer). Every one of those defaults
...
export class SamplerMachine {
  /** The channel's volume stage — `sampler.t{i}.vol` (REQ-each-slot-has-a-channel). */
  ...
  // ---- Per-slot channel (REQ-each-slot-has-a-channel): in -> tone -> vol -> pan -> samplerBus ----
  /** Unity and inert today. It is the node REQ-a-slot-can-cut-another-slot's group choke will cut, and it
  ...
  // ---- Per-slot voice window (REQ-a-hit-plays-a-window-of-the-buffer), read by `play()` at trigger time ----
  ...
  /** 0 = no group; slots sharing a group cut each other (REQ-a-slot-can-cut-another-slot). */
   ...
   *  (REQ-a-stop-cuts-in-flight-one-shots). Drums self-terminate; a user sample is any length at all. */
  ...
  setBuffer(slot: number, buf: AudioBuffer | null): void {
    ...
    // The reversed copy belongs to the buffer it was made from (REQ-a-hit-plays-a-window-of-the-buffer). Dropping
    // it here — the one convergence point every fill path already goes through
    // (REQ-set-buffer-is-the-one-door) — is what keeps "reversed" from playing the previous slot's audio.
    this.revBuffers[slot] = null;
    this.bufferListeners.emit(slot);
  }

  // ---- Per-slot channel (REQ-each-slot-has-a-channel) — ramped, so a knob drag never zippers ----
  ...
  // ---- Per-slot voice window (REQ-a-hit-plays-a-window-of-the-buffer) — plain fields, read per hit ----
  ...
  /** 0 = no group. Two slots sharing a group cut each other (REQ-a-slot-can-cut-another-slot). */
  setSlotChokeGroup(slot: number, group: number): void {
    if (this.valid(slot)) this.chokeGroup[slot] = Math.max(0, Math.round(group));
  }

  /** Mono: a slot cuts its own previous hit instead of layering (REQ-a-slot-can-cut-another-slot). */
  setSlotMono(slot: number, on: boolean): void {
    if (this.valid(slot)) this.mono[slot] = on;
  }

  /**
   * Fade one hit out and stop it, scheduled AT `when` (REQ-a-slot-can-cut-another-slot).
   *
  ...
  /** The slot's reversed copy, built on first use (REQ-a-hit-plays-a-window-of-the-buffer). */
  ...
  private play(slot: number, when: number, velocity: number, chokeAt?: number): void {
    ...
    // ---- The voice window (REQ-a-hit-plays-a-window-of-the-buffer) ----
    ...
    // ---- One cut, at whichever reason comes first (REQ-a-hit-plays-a-window-of-the-buffer) ----
    ...
    // REQ-a-slot-can-cut-another-slot — resolved BEFORE this hit joins the set, so it never cuts itself.
    ...
    // REQ-a-slots-polyphony-is-bounded — bounded polyphony. Oldest first: a Set preserves insertion order,
   ...
   * the cut never clicks (REQ-a-stop-cuts-in-flight-one-shots). The fade is on the per-hit gain, upstream of
