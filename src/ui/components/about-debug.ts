...
/** An inline action button attached to a row's value cell (REQ-a-debug-row-may-carry-one-action). */
interface RowAction {
  label: string;
  testId: string;
  onClick: () => void;
...
export function buildDebugSection(engine: StudioApi): {
  ...
  /** Every row, in order — the source for the copyable report (REQ-a-panel-level-actions-block). */
  ...
  // ---- actions (REQ-a-panel-level-actions-block) ----
  ...
  const refresh = (force = false): void => {
    ...
    // at boot it is the one-glance confirmation of why (REQ-nothing-is-audible-before-the-first-start).
    const ctxNotes = [
    ...
    // REQ-an-unbound-action-renders-disabled — an action whose source never bound is disabled, not broken.
  ...
  // Folded shut, the rows aren't merely invisible — they go unread (REQ-debug-refreshes-while-expanded):
  ...
  /** What the modal's interval and `statechange` drive — gated per REQ-debug-refreshes-while-expanded. */
  const tick = (): void => { if (visible) refresh(); };

  // REQ-nothing-an-action-starts-outlives-the-panel — nothing an action started may outlive the modal.
  const dispose = (): void => {
    stopTone?.();
    stopTone = null;
    setButtonLabel(toneBtn, 'Test tone');
