...
owner: core
related:
  - transport            # REQ-13: the step router the wrap runs through
  ...
  - arrangement
  - sequencer            # REQ-14: a tie across a wrap releases at its gate end
  - midi-clock-sync      # REQ-23: a master announces every wrap
  - meter                # a bar is `barTicks`, not 16
  - song-mode            # REQ-14: a load clears the loop
  - runtime-performance
  - onboarding
...
source:
  ...
  - src/audio/transport/clock.ts              # setStepRouter (transport.md REQ-13)
...
scrubber ([transport-window](transport-window.md) REQ-7) moves the playhead once,
...
REQ-12). That is fine once, but a loop jumps every few bars and has to land
...
- **REQ-1** — **A Loop button on both transport surfaces.** `buildTransportControls`
  renders `<prefix>-loop` between the readout and the scrubber, so the Song-panel
  row and the TRANSPORT window each carry one
  ([transport-window](transport-window.md) REQ-1). Both mirror one shared model
...
- **REQ-2** — **While Loop is on, a scrubber click picks a bar instead of seeking.**
  The first click sets an **anchor**. The second sets the range from the anchor
  to that bar, inclusive, in either order — clicking bar 3 then bar 2 loops bars
  2–3, and clicking the same bar twice loops that one bar. A new pick never
  interrupts playback: the old range keeps looping until the second click
  replaces it. With Loop off, a scrubber click seeks exactly as before.
  ...
  mitigations:
    ...
    - **Seeking stays one gesture away.** ⏮, the machine-tab rulers, `Home` and
      ...
      ([transport-position](transport-position.md) REQ-9/REQ-11). Only the
  ...
  ([transport-window](transport-window.md) REQ-4/REQ-11), at touch size
  (law 6: 44 px). On a touch screen the drag would also collide with scrolling
  the timeline sideways.

- **REQ-3** — **The loop wraps only on a bar line.** After each emitted step the
  clock asks the loop where to go next (REQ-13 of [transport](transport.md)).
  For a range of bars `[start, end]`, with `s = start·barTicks` and
  `e = (end+1)·barTicks`, the next step `n` is kept unless **all** of these hold:
  `n % barTicks === 0`, the loop is engaged and not refused (REQ-6), and
  ...
  up to the new position ([transport-position](transport-position.md) REQ-4). It
...
- **REQ-4** — **Turning a loop on while stopped moves the cue into it.** When Loop
  turns on (or the second pick completes a range) while the transport is
  stopped, and the cue lies outside the range, the cue moves to the range's first
  bar through `StudioApi.seekTo`. Play then starts inside the loop instead of
  playing an unrelated bar first. A cue already inside the range is left alone,
  so a pause inside the loop still resumes where it paused. While playing, REQ-3's
  next-bar-line rule applies instead.

- **REQ-5** — **Turning Loop off keeps the range.** Off stops the wrapping and
  clears any half-made pick (the anchor), but the range stays and is drawn
  **dimmed** on the scrubber. Turning Loop on again uses that range immediately.
  This matches the loop switch in a DAW, which remembers its range. The range is
  lost only when a song is loaded or a new song is started (REQ-10).

- **REQ-6** — **A loop that cannot jump does not jump.** A wrap is a seek, so it
  is refused in exactly the states where `canSeek()` is false
  ([transport-position](transport-position.md) REQ-6):
  ...
  ([transport-window](transport-window.md) REQ-8). Play/Pause stays available:
  *controls* stay live, *position* does not
  ([midi-clock-sync](midi-clock-sync.md) REQ-24).

- **REQ-7** — **The range is limited to the song's length without being edited.**
  The saved range is not changed when the song gets shorter. The *effective*
  range, used by both the wrap and the drawing, is limited to
...
- **REQ-8** — **A sync master announces every wrap.** A wrap is a jump, and slaves
  count pulses from their own start, so a silent wrap would leave them one loop
  behind, then two. `SyncController` subscribes to `clock.onSeek` and sends
  `songposition` + `continue` while it is master
  ([midi-clock-sync](midi-clock-sync.md) REQ-23). Moving the announce there from
  `Engine.seekTo` makes **every** jump announce itself, whether a user seek or a
  loop wrap, from one place.

- **REQ-9** — **A tied note does not hang across a wrap.** A wrap happens while
  the last step before it may still be waiting in the look-ahead. The
  sequencer's seek reaction used to release held notes *now*, which is before a
  pending note-on, and that note-on then cancelled the release. The seek reaction
  now releases at each track's own gate end, the same way its stop reaction
  already does ([sequencer](sequencer.md) REQ-14/REQ-15).

- **REQ-10** — **Loading a song clears the loop.** A range is a set of bars in
  *this* song. Every load and New already moves the playhead to bar 1
  ([song-mode](song-mode.md) REQ-14), and the same step now turns Loop off and
  forgets the range. A demo clicked while looping then plays from its first bar
  instead of looping bars that mean nothing in the new song.

- **REQ-11** — **Bars are the song's bars.** The wrap measures with `barTicks`
  ([meter](meter.md) REQ-6), which the engine sends to the loop driver from
  `applyMeter` like every other bar counter. In 3/4 a loop of bars 2–3 is ticks
  12..35. A machine whose own length is longer than a bar
  ([meter](meter.md) REQ-10) restarts with the loop, just as it does on a
  scrubber seek.

- **REQ-12** — **What the scrubber shows.** Cells inside the effective range carry
  the global `loop` class, and the anchor carries the global `loop-anchor` class.
  Both are global for the same reason `playing` and `cue` are: E2E has no other
  way past CSS-Module hashing. The scrubber carries a module class while
  picking (Loop on) and another while a remembered range is dimmed (Loop off).
  A cell may be `playing` and `loop` at once and must still read clearly. There
  are **no transitions**, since the classes move on picks, which are rare, and
  the `playing` class beside them moves every bar
  ([transport-window](transport-window.md) REQ-11). Cell classes and titles are
  redrawn on a loop change, a rebuild or an arrangement change (a shorter chain
  limits the range, REQ-7), never per tick. The arrangement notifies on every bar
  line, so the redraw first compares what it would draw with what it last drew
  and does nothing when they match.

- **REQ-13** — **Discoverability.** The Loop button's `title` (REQ-1), the cells'
  pick titles (REQ-2) and the `transport.song` help topic
  ([transport-window](transport-window.md) REQ-10) cover it. The help topic
  explains Loop, the two picks, the dimmed remembered range and when looping is
  unavailable.

- **REQ-14** — **Costs nothing unless engaged.** The clock's step router is
  `null` unless Loop is on **and** a range exists, so a transport that is not
  looping pays one null check per tick. When engaged, a tick that is not on a bar
  line costs one modulo. Only a bar line reads `songBars()` and `canSeek()`
  ([runtime-performance](runtime-performance.md)).

- **REQ-15** — **Nothing is saved.** Loop on/off, the range and the anchor exist
  only for the current session, like the cue
...
TransportLoop:   # src/audio/transport/transport-loop.ts — model only, no clock
  get enabled: boolean
  get range: LoopRange | null      # the stored range (REQ-5), unclamped (REQ-7)
  get anchor: number | null        # the first pick, awaiting the second
  get engaged: boolean             # enabled && range !== null
  setEnabled(on): void             # off also drops the anchor (REQ-5)
  toggle(): void
  ...
  clear(): void                    # off + range null + anchor null (REQ-10)
  onChange(fn: () => void): () => void   # fires only when something changed

effectiveLoopRange(range, songBars): LoopRange | null   # REQ-7; songBars 0 -> 1
routeLoopStep(next, range, barTicks): number            # REQ-3, pure; `range`
...
LoopDriver:      # src/audio/transport/loop-driver.ts
  new LoopDriver(deps: {
    ...
    canSeek(): boolean,          # Engine.canSeek (REQ-6)
    seekTo(step): boolean,       # Engine.seekTo (REQ-4)
  })
  setBarTicks(ticks): void       # pushed from Engine.applyMeter (REQ-11)
  # on loop.onChange: install the router iff engaged (REQ-14); if it just
  # became engaged (or its range changed) while stopped, REQ-4's cue move
...
driver route(n):
  ...
  return to !== n && canSeek() ? to : n     # REQ-6, checked only when jumping
```
...
clock (transport.md REQ-13): after `_step++`, router(_step); a different result
  sets _step, fires onSeek (each listener isolated), cue + nextStepTime untouched
wrap fan-out (same order as a seek, transport-position REQ-3):
...
song-panel toTop: loop.clear() THEN seekTo(0)   # clear first, so REQ-4 cannot
...
defect. REQ-2 records why this one exception is accepted and what keeps the state
...
```gherkin
Scenario: Two picks loop the bars between them (REQ-2, REQ-3)
...
Scenario: Picks are order-independent, and one bar is a valid loop (REQ-2)
  Given Loop is on
...
Scenario: The old range keeps looping until the second pick lands (REQ-2)
...
Scenario: Loop on turns a scrubber click into a pick, not a seek (REQ-2)
  Given Loop is on
...
Scenario: Engaged while playing outside the range, it goes in at the bar line (REQ-3, edge)
...
Scenario: A wrap does not move the cue (REQ-3)
...
Scenario: Engaging while stopped cues the loop (REQ-4)
...
Scenario: Off remembers the range; on reuses it (REQ-5)
...
Scenario: A refused loop is suspended, never applied (REQ-6)
...
Scenario: A shorter song limits the loop without editing the range (REQ-7, edge)
...
Scenario: A sync master announces each wrap (REQ-8)
...
Scenario: A tie into the wrap releases at its gate end (REQ-9, regression)
...
Scenario: Loading a song clears the loop (REQ-10)
...
- **A slave follows a wrap by re-joining.** REQ-8 reuses REQ-23's
