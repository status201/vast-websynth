...
version: 8   # v8: REQ-1/9/10/11 — a second transport (Streamable HTTP) and the
             ...
             # v5: REQ-get-params-is-shared-by-both-halves — get_params; baseUrl defaults to the published site
             # v4: REQ-an-mcp-write-stays-in-the-working-directory — save_song/save_preset contain `dir` inside cwd
...
source:
  - scripts/mcp/websynth-mcp.mjs      # local entry: stdio framing (REQ-stdio-is-newline-delimited-json-rpc)
  ...
  - src/state/limits.ts               # MAX_MCP_* — the bounds, per untrusted-input REQ-the-limits-are-one-module
...
boundary of a public endpoint (REQ-the-public-endpoint-is-bounded-not-authenticated) and nothing automated will stop an edit
...
- **REQ-two-transports-one-dispatcher** — (v8) **Two transports, one
  dispatcher.** `createDispatcher` (`rpc.mjs`) maps a parsed JSON-RPC message to
  a response object and knows nothing about framing; both transports are thin
  shells around it, and REQ-initialize-echoes-the-protocol-version's protocol
  behaviour is therefore identical on each. A transport chooses only two things:
  how a message arrives, and which tool profile it serves
  (REQ-the-remote-profile-is-read-only).

- **REQ-stdio-is-newline-delimited-json-rpc** — **stdio** (local):
  newline-delimited JSON-RPC 2.0, one JSON object per line, per the MCP spec.
  `stdout` carries **protocol frames only**; every log/diagnostic goes to
  `stderr` — including anything Vite would print during the self-build, which
  cannot reach the protocol stream at all: the build child's `stdout` *and*
...
- **REQ-streamable-http-is-one-message-per-post** — **Streamable HTTP**
  ...
  SSE and no session (REQ-the-http-transport-is-stateless). Logs go to
...
- **REQ-initialize-echoes-the-protocol-version** — `initialize` echoes the
  client's `protocolVersion` when the server knows it, else answers
  `"2025-06-18"`; capabilities are `{tools: {}}`. `notifications/initialized`
  (and all notifications) get no response; `ping` returns `{}`; unknown methods
  get `-32601`; a malformed line gets `-32700`.

- **REQ-local-entries-self-build-the-core** — The **local** entries self-build
  `scripts/mcp/dist/song-core.mjs` when the bundle is missing or older than any
  file under `scripts/mcp/song-core-entry.ts`, `src/state/**` or `src/utils/**`.
  A clean checkout therefore needs only `npm install` before registering the
  server. The build runs in a **child process** (`node
  node_modules/vite/bin/vite.js build --config …`), never in the server itself:
  Vite bundles with rolldown, whose native binding the OS locks for the lifetime
  of the process that loaded it. In a server that outlives the client's session,
  an in-process build pins `node_modules` for hours and the user's next `npm ci`
  fails with `EPERM` on a file no tool names. Nothing long-lived here may load a
  native module. A missing `vite` binary fails fast, naming `npm install`. (v8)
  The check and the build live in `core.mjs` as `loadCore({selfBuild})`, shared
  by both local entries. **The deployed entry passes `selfBuild: false`** and
  *throws* on a missing bundle instead of building one: production has no
  `node_modules` to build from, and a server that shells out to a bundler on an
  unauthenticated request would be a far worse thing than a server that refuses
  to start.

- **REQ-song-core-entry-exports-only-pure-code** — `song-core-entry.ts`
  re-exports only the **pure** core: `validateSongFile`,
  `isAuthorSong`/`expandAuthorSong`, `compactSongForExport`,
  `buildAuthoringGuide`, `ParamBus`/`registerDefaults`, plus (v2) the preset
  half — `validatePresetPayload`, `expandPresetParams`, `defaultPatchParams`,
  `buildPresetGuide`, `buildPresetFile`/`buildBankFile`,
...
- **REQ-five-song-tools** — Five song tools, all built by `makeTools(core)` with
  the core **injected** (unit-testable without the bundle):
  ...
  - `expand_song` — author dialect → canonical compact JSON, at the lowest
    version that holds the content (song-authoring-dialect.md REQ-the-emitted-version-is-the-lowest-that-fits). The tool
  ...
  - `make_share_link` — canonical compact JSON → `node:zlib.deflateRawSync` →
    ...
    base URL from `$WEBSYNTH_BASE_URL`, default the published site (REQ-base-url-defaults-to-the-published-site).

- **REQ-four-preset-tools** — (v2) Four **preset** tools, same shape and same "a
  failed validation is a successful call" rule (see
...
- **REQ-an-mcp-write-stays-in-the-working-directory** — (v4) **A write stays
  inside the working directory.** Both `save_song` and `save_preset` sanitize
  the *filename* (`safeName`) **and** contain the `dir` argument: `resolve(cwd,
  dir)` must remain under `cwd` or the call fails without writing
  (`isAbsolute(relative(cwd, target))` is what catches another Windows drive
  letter, where `relative` returns an absolute path rather than `..`). `dir` is
  already documented as "the server working directory", so containment is the
  contract — it just wasn't enforced, and `mkdirSync(…, {recursive:true})` would
  happily create the path on the way out. The caller is a model, and a model
  summarising a hostile song file is a prompt-injection route to an arbitrary
  file write ([untrusted-input](untrusted-input.md)
  REQ-mcp-writes-stay-in-the-working-directory).

- **REQ-get-params-is-shared-by-both-halves** — (v5) One **shared** tool ahead
  of both halves: `get_params` returns `buildParamCatalog(bus)` as JSON
  ([param-catalogue](param-catalogue.md) REQ-mcp-serves-the-catalogue). The two
  `get_*_format` tools serve *prose* — an agent that wants a range to compute
  against had to parse a comment out of the PARAMS table, and `taper`, `curve`
  and `unit` were not in that text at all. It shares the same per-process bus as
  the rest.

- **REQ-base-url-defaults-to-the-published-site** — (v5) `baseUrl` defaults to
  the **published site**, not `http://localhost:5173`. Every schema URL the
  guides cite is built from it, so the old default handed the model dead links
  unless a dev server happened to be running — the one thing a *published*
  schema must not do. `$WEBSYNTH_BASE_URL` still overrides, which is how a local
  dev server or a fork's host is pointed at.

- **REQ-tool-input-errors-are-json-rpc-errors** — Tool *input* errors (unknown
  tool, missing argument) are JSON-RPC errors; tool *runtime* failures return
  `isError: true` with the message in `content` (per MCP). Unparseable `song`
  JSON strings count as validation results (`{ok:false, errors:[…]}`), not
  crashes.

- **REQ-mcp-json-registers-the-server** — `.mcp.json` at the repo root registers
  the server for Claude Code (`node scripts/mcp/websynth-mcp.mjs`); the README
  documents wiring for other MCP clients. `scripts/mcp/dist/` is gitignored.

- **REQ-a-valid-song-can-still-be-wrong** — (v6) **A song that validates can
  still be wrong, and the agent is told.** `validate_song` returns `{ok,
  errors[], warnings[]}` and `save_song` adds `warnings[]` to its success
  payload, carrying [untrusted-input](untrusted-input.md)
  REQ-an-unresolvable-target-warns's unresolvable automation targets. This
  closes the authoring loop's one silent hole: an `xy` or `motionTracks` target
  naming a parameter that does not exist used to come back `{ok:true,
  errors:[]}` and then move nothing at play time, which an agent has no way to
  discover — it cannot hear the result. `expand_song` keeps returning the song
  **text alone** (its output is parsed, so a wrapper object would be a breaking
  change); its description points at `validate_song` for warnings. Warnings
  never set `ok:false` — [untrusted-input](untrusted-input.md)
  REQ-an-unresolvable-target-warns owns why.

- **REQ-the-http-transport-is-stateless** — (v8) **The HTTP transport is
  Streamable HTTP, stateless.** `createRequestListener({dispatch, version,
  origins, limits})` returns a `(req, res)` handler — no socket needed to test
  it. The contract:
  ...
  There is deliberately **no `404`**. REQ-the-request-path-is-not-matched means the path is not routed on, so
  the server has no notion of a path being "wrong" — every non-`POST` is
  answered by method, not by location.

- **REQ-no-mcp-session-id-is-issued** — **No session.** No `Mcp-Session-Id` is
  ever issued. There is no per-connection state to key one on: the single
  lazily-created `ParamBus` inside `makeTools` is read-only and shared per
  process. The MCP spec permits the omission, and it means no session table to
  bound and no eviction policy.

- **REQ-no-sse-every-response-is-one-json-body** — **No SSE.** Every response is
  a single JSON body, so nothing depends on a reverse proxy declining to buffer.
  `GET` answering `405` is the spec's sanctioned way to say "this server offers
  no server-initiated stream", and it doubles as the reason the service worker
  can never cache this path ([pwa-install](pwa-install.md)
  REQ-service-worker-is-registered — a 405 is not a cacheable 200).

- **REQ-an-absent-origin-is-allowed** — **`Origin` absent is allowed; `Origin`
  present must match.** Claude calls server-side and sends none, so requiring
  the header would break the only client that matters. When a browser *does*
  send one it must be in the allowlist (the published site, plus loopback for
  the MCP Inspector), else `403`. That is the spec's DNS-rebinding defence, and
  it is a browser-only concern by construction.

- **REQ-the-protocol-version-header-is-ignored** — **`MCP-Protocol-Version` is
  ignored.** Not read and discarded — never consulted. `rpc.mjs` behaves
  identically at every revision it knows and `initialize` already negotiates
  (REQ-initialize-echoes-the-protocol-version), so there is nothing the header
  could change; validating it would only give a client newer than this file a
  way to be refused for no reason.

- **REQ-the-request-path-is-not-matched** — **The path is not matched.** Any
  path accepts the `POST` (`/healthz` excepted), because the same process is
  reached at `/mcp` behind the production proxy, at `/` on the origin server,
  and at whatever a developer types locally. Hard-matching `/mcp` would make the
  deployment topology a code constant.

- **REQ-the-remote-profile-is-read-only** — (v8) **The remote profile is
...
- **REQ-the-public-endpoint-is-bounded-not-authenticated** — (v8) **The public
  ...
  bounds are the whole defence, and they live in `src/state/limits.ts` like
  every other bound in this repo ([untrusted-input](untrusted-input.md)
  REQ-the-limits-are-one-module/REQ-the-public-endpoint-is-bounded):
  `MAX_MCP_REQUEST_BYTES`, `MAX_MCP_REQUESTS_PER_MINUTE`, `MAX_MCP_RATE_KEYS`,
  `MAX_MCP_REQUEST_MS`. Three rules make them real:
...
tools.mjs:
  makeTools: '(core, {baseUrl?, cwd?, allowWrites?}?) => Tool[]'   # allowWrites defaults true (REQ-the-remote-profile-is-read-only)
core.mjs:
  loadCore: '({selfBuild}?) => Promise<Core>'   # selfBuild:false throws on a missing bundle (REQ-local-entries-self-build-the-core)
http.mjs:
  createRequestListener: '({dispatch, version, origins?, limits?}) => (req, res) => void'
song-core-entry.ts: 're-exports the pure core (REQ-song-core-entry-exports-only-pure-code) + the MAX_MCP_* limits'
...
Order matters in one direction only: the bounds (REQ-the-public-endpoint-is-bounded-not-authenticated) are applied **before**
...
(REQ-the-remote-profile-is-read-only), so the deployed server touches no disk at all after boot.
...
Scenario: expand_song returns canonical compact JSON
  ...
  Then the text payload parses as format "websynth-song", version 3
       (that song uses no post-v3 field — see song-authoring-dialect.md REQ-the-emitted-version-is-the-lowest-that-fits;
...
Scenario: A dead automation target comes back as a warning (v6, REQ-a-valid-song-can-still-be-wrong)
...
Scenario: The same handshake works over HTTP (v8, REQ-two-transports-one-dispatcher/REQ-the-http-transport-is-stateless)
...
Scenario: A notification over HTTP is accepted, not answered (v8, REQ-the-http-transport-is-stateless)
  When a client POSTs notifications/initialized
  Then the response is 202 with an empty body
# pinned by: tests/mcp/http.test.ts

Scenario: The remote profile has no write tools (v8, REQ-the-remote-profile-is-read-only)
  When tools/list runs against the HTTP transport
  Then it lists exactly the eight read-only tools, in REQ-the-remote-profile-is-read-only's order
...
Scenario: The local profile is unchanged by the remote one (v8, REQ-the-remote-profile-is-read-only)
  When makeTools is called with no allowWrites option
  Then it returns all ten tools in the original order
# pinned by: tests/mcp/tools.test.ts, tests/mcp/integration.test.ts

Scenario: An oversized body dies in transit (v8, REQ-the-public-endpoint-is-bounded-not-authenticated)
...
Scenario: A flood is rate-limited per IP (v8, REQ-the-public-endpoint-is-bounded-not-authenticated)
...
Scenario: The rate limiter cannot itself be made to leak (v8, REQ-the-public-endpoint-is-bounded-not-authenticated)
...
Scenario: Claude's own request has no Origin, and is allowed (v8, REQ-an-absent-origin-is-allowed)
  When a POST arrives with no Origin header at all
  Then it answers 200
  But a POST with Origin "https://evil.example" answers 403
# pinned by: tests/mcp/http.test.ts

Scenario: GET offers no stream, so nothing can cache it (v8, REQ-no-sse-every-response-is-one-json-body)
...
Scenario: A rebuild leaves node_modules replaceable (v7, REQ-local-entries-self-build-the-core, regression)
...
- Integration: `tests/mcp/integration.test.ts` — deletes `scripts/mcp/dist`,
  spawns the server so it self-builds (REQ-two-transports-one-dispatcher/REQ-local-entries-self-build-the-core), then drives initialize →
...
- **Half of REQ-local-entries-self-build-the-core stays unpinned.** `tests/mcp/integration.test.ts` now starts
...
- A `render_song` tool once an offline render path exists outside the browser.
  ...
  also the first thing that would force REQ-no-sse-every-response-is-one-json-body's no-SSE decision to be revisited
