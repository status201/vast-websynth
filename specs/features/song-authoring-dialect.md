...
- **REQ-12** — **The emitted canonical version is the LOWEST that can hold what
  ...
  **7** when any chain slot carries a non-zero transpose (REQ-15), else **6**
  when any seq bank uses tracks 2-4 (REQ-13), else **5** when `motionTracks` is
  present (REQ-14), else **4** when motion content is present, else **3**.
  Consequence — and the reason for the rule — an author file that
...
# pinned by: tests/state/song-author.test.ts (machines with hits auto-enable — REQ-11)
...
Scenario: bank-level settings cascade into every track (REQ-13b)
...
Scenario: a non-setting key beside tracks is an error (edge — REQ-13)
