...
version: 10                 # v10: `otherLfo` removed from lfo-routing.ts — nothing called it
                            ...
                            #     unchanged.
                            # v8: REQ-destinations-are-no-longer-exclusive's mutual exclusion is SUPERSEDED by the mod
                            #     matrix; the two LFOs become its rows 0-1, keeping
                            #     lfo.dest / lfo.amount unchanged (mod-matrix.md)
                            # v7: REQ-there-are-two-lfos..15 — a second LFO (lfo2.*), mutually
                            #     exclusive destinations, arbitrated PWM, tab pages
                            # v6: REQ-lfo-sync-locks-rate-to-tempo — lfo.sync locks the rate to the tempo
                            # v5: lfo.rate is exponentially tapered (REQ-lfo-rate-is-exponentially-tapered)
...
related:
  ...
  - dropdown             # v8: greying now serves mod-matrix REQ-per-voice-sources-cannot-drive-bus-destinations, not REQ-destinations-are-no-longer-exclusive
  - mod-matrix           # v8: the LFOs are its rows 0-1; supersedes REQ-destinations-are-no-longer-exclusive
  - tempo-lock           # v9: owns REQ-lfo-sync-locks-rate-to-tempo's control; the effects share it now
source:
  - src/audio/lfo.ts
...
destinations, genuinely cannot be shared (REQ-pulse-is-arbitrated).

## Requirements

- **REQ-each-lfo-has-four-controls** — Each LFO has rate / amount / waveform /
  destination.
- **REQ-mod-wheel-sums-into-lfo-one** — LFO 1's effective amount = `min(1,
  lfo.amount + master.modWheel)`; **both** `lfo.amount` and `master.modWheel`
  recompute it. LFO 2's effective amount is `lfo2.amount` alone — see
  REQ-the-mod-wheel-feeds-lfo-one-only.
- **REQ-destination-is-one-of-the-labels** — Destination is one of the
  `LFO_DEST_LABELS` (range `0..6`, v4). Labels are **append-only**: an index is
  a stored value in every preset, song and share link, so reordering silently
  rewrites saved patches.
- **REQ-pan-sweeps-a-stereo-panner** — (v2) The `pan` destination sweeps a
  single `StereoPannerNode` on the **synth bus**, placed after the insert chain
  and before `preMaster`. At full amount the sweep is hard L↔R;
  `StereoPannerNode.pan` clamps to `±1` by construction, so the destination is
  bounded for any amount. Because the LFO is global (one oscillator shared by
  all voices), a bus panner and a per-voice panner are audibly identical here —
  the bus node is chosen so that only one node pans, instead of one per voice,
  and so that the insert chain upstream of the reverb stays 1-channel (ADR-010,
  *cheap*; same reasoning as [ladder-filter](ladder-filter.md)
  REQ-the-filter-worklet-is-mono). The reverb itself is mono-in / stereo-out — a
  2-channel decorrelated IR — so it, not the panner, is where the synth channel
...
- **REQ-amplitude-destinations-are-smoothed** — (v2) The **amplitude-domain**
  ...
  `cutoff`) are fed from the oscillator directly: a stepped octave or filter
  jump is a musical event, not a click. At LFO rates the smoothing is inaudible
  on shape (−0.02 dB, ~6° at 20 Hz). Each LFO owns its own smoothing filter.
...
- **REQ-pulse-destination-sweeps-width** — (v3) The `pulse` destination sweeps
  oscillator **pulse width**. It is the one destination with **no audio-node
  output**: a native `OscillatorNode` has no width `AudioParam`, so it is driven
  by `PwmDriver` from a JS-side mirror of the LFO shape rather than by a gain
  node. Its contract, rate cap and cost budget live in
  [oscillators](oscillators.md) REQ-width-swaps-a-precomputed-wave and the four
  PWM rules after it, ending at oscillators.md
  REQ-a-background-tab-throttles-pwm. Because one LFO holds one destination,
  that LFO's JS mirror and its own audio oscillator are never both in use, so
  they cannot drift against each other in any way a listener can hear — still
  true with two LFOs, since each owns its own oscillator and only the driver's
  owner runs a mirror (REQ-pulse-is-arbitrated).
- **REQ-shape-destination-sweeps-the-pole-mix** — (v4) The `shape` destination
  sweeps the POLY filter's pole-mix morph, `filter.shape`
  ([filter-models.md](filter-models.md)
  REQ-shape-morphs-the-pole-mix/REQ-shape-is-an-lfo-destination) — the LFO
  ...
  **smoothed** path like `amp`/`pan`, not the raw path like `cutoff`: it moves
  ...
  `filter.shape` (filter-models REQ-shape-is-poly-only).
- **REQ-lfo-rate-is-exponentially-tapered** — (v5) `lfo.rate` is **exponentially
  tapered**. Rate is perceived in octaves, not in Hz: linearly, the musically
  useful sub-1 Hz region occupied the first ~5% of the knob's travel (~9 px of a
  200 px drag) while half the dial was spent between 10 and 20 Hz. `exp` spreads
  it evenly — each equal turn is a constant *ratio*, so 0.05→0.5→5 Hz are
  equally spaced, matching how every hardware LFO rate pot is wired. `min = 0.05
  > 0`, which `exp` requires.
...
- **REQ-lfo-sync-locks-rate-to-tempo** — (v6) **`lfo.sync` locks the rate to the
  tempo.** A free-running LFO drifts against the song: set a 3 Hz wobble at 120
  BPM, change to 128, and it no longer lines up with anything. `lfo.sync` is a
  discrete param whose index 0 is `free` — **the default, and an exact no-op** —
  followed by the note divisions from `utils/tempo.ts` (`1/1`, `1/1 D`, `1/1 T`,
  … `1/32 T`). Rules:
  ...
  - ~~The rate knob **dims** while synced~~ — **superseded in v9.** The knob now
    ...
    the chosen division replaces the dial ([tempo-lock](tempo-lock.md) REQ-the-lock-is-a-note-glyph/REQ-locked-the-division-replaces-the-dial).
  ...
  - `pwm.setRate` follows the same effective rate, since PWM rides the LFO
    (REQ-pulse-destination-sweeps-width). With two LFOs, "the LFO" means the one that owns the driver
    (REQ-pulse-is-arbitrated).

- **REQ-there-are-two-lfos** — (v7) **There are two LFOs**, `lfo.*` and
  `lfo2.*`, identical in every respect except
  REQ-the-mod-wheel-feeds-lfo-one-only. Both param sets are produced by **one
  ...
  `LFO_DEST_LABELS` and `SYNC_LABELS` verbatim
  (REQ-destination-is-one-of-the-labels's append-only rule covers both), and v9
  shares `syncParam(prefix)` — the same `.sync` def the effects get
  ([tempo-lock](tempo-lock.md) REQ-sync-defaults-to-free) — so the guarantee now
  spans the LFOs *and* the wah/phaser/delay rather than the pair alone.
  `lfo2.dest` defaults to `0` (`off`) **and** `lfo2.amount` to `0` — a double
  no-op, so every preset, song and share link that predates v7 sounds identical
  (ADR-006). Being additive scalar params, they need **no song-format version
  bump** (ADR-007); `SONG_VERSION` stays 7.

- **REQ-the-mod-wheel-feeds-lfo-one-only** — (v7) **The mod wheel feeds LFO 1
  only.** `master.modWheel` sums into `lfo.amount`
  (REQ-mod-wheel-sums-into-lfo-one) and does not reach `lfo2.amount`, which is
  set by its knob alone. This is the one deliberate asymmetry between the two
  LFOs. The wheel is a performance gesture with one classic meaning — bring in
  the vibrato — and a wheel that simultaneously opens a second, unrelated
  modulation is a single gesture with two outcomes (ADR-014 law 2). LFO 2 is the
  patched, set-and-forget modulator; LFO 1 is the played one.

- **REQ-destinations-are-no-longer-exclusive** — (v7) ~~**Destinations are
  mutually exclusive, in the UI.**~~ **Superseded in v8 by
  [mod-matrix](mod-matrix.md) REQ-two-rows-may-share-a-destination.** The two
  LFOs may now hold the same destination, and it sums — which
  REQ-duplicated-destinations-sum-and-stay-bounded below already specified and
  the audio graph already did. `blockedDests` is deleted along with the
  `paramHint` that named the holder; `lfo-routing.ts` keeps the prefix
  vocabulary the two-page panel still needs.
  ...
  **Removing the block therefore cannot change how any saved sound plays**; it only
  ...
  [mod-matrix](mod-matrix.md) REQ-per-voice-sources-cannot-drive-bus-destinations greys a **bus-wide** destination while a
...
- **REQ-duplicated-destinations-sum-and-stay-bounded** — (v7) **Duplicated
  destinations sum, and stay bounded.** Every destination except `pulse` is a
  `GainNode` into a summing `AudioParam`, so a hand-authored file with both LFOs
  on one destination is well-defined rather than undefined: pitch `±2400` cents,
  cutoff `±48` semitones (still additive in semitone space — ADR-005 holds),
  shape `±1.0` around the knob, amp `0.0..2.0` around the tremolo VCA's base
  `1.0` (the trough touches silence exactly and never inverts), and pan clamped
  to `±1` by `StereoPannerNode` (REQ-pan-sweeps-a-stereo-panner's bounding
  argument holds unchanged for two sources).

- **REQ-pulse-is-arbitrated** — (v7) **`pulse` is arbitrated, because it is a
  parameter write and not a summed connection.** There is one shared `PwmDriver`
  (REQ-pulse-destination-sweeps-width), and it takes a **source index** and
  tracks an owner:
...
- **REQ-the-two-lfos-share-one-panel** — (v7) **The two LFOs share one panel, as
  two tab pages.** The faceplate is an 8-column grid and all eight columns are
  taken, so LFO 2 is a second page in the LFO panel rather than a ninth panel.
  The [panel-tabs](panel-tabs.md) strip **is** the panel's header — `LFO 1` and
  `LFO 2` split the row evenly and there is no separate `LFO` title, so the pair
  costs no more vertical space than any other panel's heading (panel-tabs
  REQ-lfo-sync-locks-rate-to-tempo/REQ-there-are-two-lfos). Both pages stay in
  the DOM with live subscriptions, so the hidden one is already correct when
  revealed. The selected page is **session-only view state**: never a param,
  never persisted (panel-tabs.md REQ-selected-page-is-session-only). Because a
  modulating LFO on the hidden page would otherwise be invisible state, its tab
  carries a **lamp** whenever `dest !== off && amount > 0` (ADR-014 law 5) —
  which is where the unselected tab's dark well earns its keep, since that is
  the tab the lamp has to be legible on.
...
factory for `prefix` in `lfo`, `lfo2` (REQ-there-are-two-lfos):

```yaml
<prefix>.rate:   { range: 0.05..20, default: 4, format: Hz, taper: exp }  # v5, REQ-lfo-rate-is-exponentially-tapered
...
<prefix>.sync:   { discrete, labels: SYNC_LABELS, range: 0..18, default: 0 }     # v6, REQ-lfo-sync-locks-rate-to-tempo
master.modWheel: { range: 0..1, default: 0 }            # sums into LFO 1 only (REQ-the-mod-wheel-feeds-lfo-one-only)
```
...
through the shared `bindTempoLocked` ([tempo-lock](tempo-lock.md) REQ-audio-resolves-the-lock-in-one-place) — PWM
rides the same callback, so it cannot disagree with the oscillator about the
effective rate (REQ-pulse-destination-sweeps-width/REQ-pulse-is-arbitrated) — which is LFO 1 only (REQ-the-mod-wheel-feeds-lfo-one-only).
...
# blockedDests(mine, theirs) — DELETED with REQ-destinations-are-no-longer-exclusive (v8)
```

What greys a destination now is the per-voice/bus-wide rule in
[mod-matrix](mod-matrix.md) REQ-per-voice-sources-cannot-drive-bus-destinations, which lives in `src/state/mod-routing.ts` in exactly
...
engine (subscribeParams):                    # v7: two bind calls replace the block
  ...
  this.lfo2.bind(bus, 'lfo2', this.pwm, 1)   # no mod wheel — REQ-the-mod-wheel-feeds-lfo-one-only
...
Scenario: The rate knob moves in octaves, not in Hz (v5, REQ-lfo-rate-is-exponentially-tapered)
...
Scenario: An existing patch's stored rate survives the taper change (v5, REQ-lfo-rate-is-exponentially-tapered)
...
# pinned by: tests/state/params.test.ts
Scenario: A synced LFO takes its rate from the tempo (v6, REQ-lfo-sync-locks-rate-to-tempo)
...
Scenario: Free-running is the default and nothing changes (v6, REQ-lfo-sync-locks-rate-to-tempo, ADR-006)
  Given lfo.sync is 0
  When the tempo changes
  Then the LFO keeps the rate the knob set
# pinned by: e2e/lfo-sync.spec.ts

Scenario: Leaving sync restores the knob's own rate (v6, REQ-lfo-sync-locks-rate-to-tempo)
...
Scenario: The rate knob shows the division while synced (v9, REQ-lfo-sync-locks-rate-to-tempo)
...
Scenario: LFO 2 changes nothing until it is armed (v7, REQ-there-are-two-lfos, ADR-006)
...
Scenario: The two LFOs are registered identically (v7, REQ-there-are-two-lfos)
...
Scenario: The mod wheel opens LFO 1 and leaves LFO 2 alone (v7, REQ-the-mod-wheel-feeds-lfo-one-only)
...
Scenario: Both LFOs may now hold one destination (v8, REQ-destinations-are-no-longer-exclusive superseded)
  Given lfo.dest is "cutoff"
  When the user opens LFO 2's destination list
  Then "cutoff" is selectable, not greyed
  And choosing it sums the two, as REQ-duplicated-destinations-sum-and-stay-bounded always specified
...
Scenario: Two LFOs on one destination sum, bounded (v7, REQ-duplicated-destinations-sum-and-stay-bounded, edge)
  Given both LFOs are routed to "pan" at full amount
  Then both toPan gains reach 1 and the panner clamps the sum to +/-1
# pinned by: tests/audio/lfo.test.ts

Scenario: LFO 2 leaving a destination does not stop LFO 1's PWM sweep (v7, REQ-pulse-is-arbitrated)
...
Scenario: The lower LFO index wins the pulse driver (v7, REQ-pulse-is-arbitrated, edge)
...
Scenario: A modulating LFO on the hidden page lights its tab (v7, REQ-the-two-lfos-share-one-panel)
...
- **By ear** (ADR-010, `recipes/verify-audio-by-ear.md`): the smoothing constants
  in REQ-amplitude-destinations-are-smoothed are only settled by listening — square at full depth on both `amp` and
  ...
  pre-v7 baseline (the REQ-there-are-two-lfos no-op); LFO 1 → `pulse` while LFO 2 is toggled
  between `cutoff` and `off` must not drop the PWM sweep (REQ-pulse-is-arbitrated); and both LFOs
  on `amp` at full depth must bottom out at silence smoothly, not click or invert
  (REQ-duplicated-destinations-sum-and-stay-bounded).
...
- **Two is a hard-coded count, not a pool** — and it stayed that way. The matrix
  ...
  destinations per source, depth per route, REQ-destinations-are-no-longer-exclusive gone (v8). What it did **not**
