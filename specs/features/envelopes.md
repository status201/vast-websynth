...
id: envelopes
status: implemented
version: 3   # v3: REQ-filter-env-follows-velocity — the filter envelope can follow velocity
...
- **REQ-amp-env-drives-voice-gain** — Amp envelope (`env.amp.*`) drives
  per-voice gain; release frees the voice.
- **REQ-filter-env-is-additive-semitones** — Filter envelope (`env.fil.*`)
  modulates cutoff additively in semitones, scaled by `filter.envAmount`
  (`-48..48`, bipolar).
- **REQ-envelopes-are-per-voice** — Both apply per-voice (each `Voice` owns its
  `ampEnv` / `filEnv`).
- **REQ-envelope-scheduling-is-future-time-safe** — Scheduling is
  **future-time-safe**: `trigger`/`release_` may be called with an audio time up
  to the transport look-ahead (0.1–0.2 s) in the future (sequencer/arpeggiator
  schedule a step's attack *and* its `gateEnd` release on the same tick). The
  phase change anchors at the value the curve *reaches at that time* — never the
  live `AudioParam.value`, which is a stale snapshot of *now* and steps the
  output discontinuously (an audible click). The envelope is the **sole writer**
  of its gain param; `Voice.kill` cuts through `Envelope.cutFast`, never by
  scheduling on the param directly.

- **REQ-filter-env-follows-velocity** — **The filter envelope can follow
  velocity (v3).** `Voice.noteOn` triggered the amp envelope at the note's
  velocity and the filter envelope at a hard-coded `1`, so **playing harder got
  louder and never brighter** — the single most-expected behaviour of a
  subtractive synth, and the one it did not have.
...
filter.velAmount:{ range: 0..1,     default: 0 }   # v3, REQ-filter-env-follows-velocity — 0 = velocity does
                                                   #   not touch the filter (ADR-006)
```

### Scheduled-automation model (REQ-envelope-scheduling-is-future-time-safe)
...
Scenario: Velocity leaves the filter alone by default (v3, REQ-filter-env-follows-velocity, ADR-006)
...
Scenario: A harder note opens the filter further (v3, REQ-filter-env-follows-velocity)
...
Scenario: Partial velocity tracking interpolates (v3, REQ-filter-env-follows-velocity, edge)
...
- `tests/state/params.test.ts`, `tests/audio/envelope.test.ts` (scheduled-automation
  model, REQ-envelope-scheduling-is-future-time-safe), `e2e/controls.spec.ts`.
