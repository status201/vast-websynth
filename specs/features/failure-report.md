...
id: failure-report
status: implemented
version: 3   # v3: REQ-truncated-message-is-built-here — the truncated message an alert shows is built here
             #     too, so the line naming the Copy button cannot drift
             # v2: REQ-every-failure-surface-emits-one covers every import-failure SURFACE, not only the
...
[dialog](dialog.md) REQ-an-alert-may-offer-copyable-text. It follows the precedent set by the Debug panel's
**Copy report** ([debug-panel](debug-panel.md) REQ-a-panel-level-actions-block): the payload is assembled
...
- **REQ-report-carries-every-message** — `buildFailureReport` takes the
  **complete** message array and returns a single string. It never truncates,
  never re-orders, and never reads the DOM. A surface that shows the user a
  shortened list still passes the whole array here — the divergence between what
  is *shown* and what is *copied* is the entire point of the facility.
- **REQ-report-opens-with-a-context-header** — The report opens with a context
  header, so a paste into a chat window or an issue identifies itself without
  further questions: 1. `VAST G1-J8 <__APP_VERSION__> — <title>` (the title is
  the dialog's own, so the report says which failure it is), 2. an ISO-8601
  timestamp (`new Date().toISOString()`), 3. `file: <name>` — **omitted
  entirely** when the caller has no file or link name to give, 4. a count line,
  then a blank line, then the messages as `• ` bullets.
- **REQ-count-line-admits-the-cap** — The count line is honest about the
  producer's own cap. It reads `1 error:` / `N errors:`, and gains ` (validator
  cap reached)` when the caller passes `capped: true` — which the song-import
  surface derives from `errors.length >= MAX_ERRORS`. Without it a 50-error
  report would claim to be complete when the validator had in fact stopped
  walking the payload.
- **REQ-report-builders-are-pure** — The builders are **pure and synchronous**:
  same inputs, same string, apart from the timestamp. They perform no clipboard
  write and raise no dialog — callers hand the result to `alertDialog`'s
  `copyable` (dialog REQ-an-alert-may-offer-copyable-text), which owns the `copyText` / `flashCopied` mechanics
  from `src/ui/clipboard.ts`. That purity is why the *composition* of "build the
  message, build the report, raise the alert" lives at its call site rather than
  here.
- **REQ-every-failure-surface-emits-one** *(v2: surface, not alert)* — Every
  import-failure **surface** in the app emits one, so a user who hits any of
  them has the same recourse — including the preset wizard's inline error strip
  ([presets](presets.md) REQ-the-preset-wizard-reports-every-problem), which is a `div` in a `Modal` rather than an
  `alertDialog` and so reuses `copyText` / `flashCopied` directly with its own
  testid. The emitting surfaces are the table under "Layer touchpoints" below; a
  *signpost* alert that carries no diagnosis (the wrong-door "That is a preset
  file" notice) deliberately does not.
- **REQ-truncated-message-is-built-here** *(v3)* — **The truncated message is
  built here too.** `failureMessage(lead, errors)` bullets the first 8 and, when
  more remain, adds the line that **names the Copy button** — the promise
  [song-mode](song-mode.md) REQ-a-rejected-import-is-copyable-in-full makes. It is one function because two
  hand-written copies drift, and a drifted copy points the user at a control by
  a name it no longer has. `isCapped` is shared for the same reason: every
  caller derived `errors.length >= MAX_ERRORS` identically, so it is derived
  once.
  ...
  feed (REQ-report-builders-are-pure).
...
```yaml
via alertDialog copyable (dialog REQ-an-alert-may-offer-copyable-text):
  src/ui/panels/song-panel.ts:
    ...
    'Demo failed to load' -> showDemoFailure: the parse's whole error array;
                             the catch's exception message otherwise  # song-mode REQ-a-rejected-import-is-copyable-in-full v25
...
```gherkin
Scenario: The report carries every message, not the truncated view (REQ-report-carries-every-message)
...
Scenario: The header identifies the failure without further questions (REQ-report-opens-with-a-context-header)
...
Scenario: The line naming the Copy button is written once (v3, REQ-truncated-message-is-built-here)
...
Scenario: The count line admits when the validator stopped early (REQ-count-line-admits-the-cap)
...
- *(closed in v2)* The Preset Import wizard's inline error strip rendered
  ...
  [presets](presets.md) REQ-the-preset-wizard-reports-every-problem.
