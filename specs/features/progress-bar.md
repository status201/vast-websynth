...
The export dialog ([audio-export](audio-export.md) REQ-the-modal-is-the-renders-own-surface) built its own progress
...
- **REQ-determinate-announces-value** (determinate) — `createProgressBar()`
  returns a `role="progressbar"` track with `aria-valuemin="0"` /
  `aria-valuemax="100"` and a fill child. `set(ratio)` clamps to 0…1, sets the
  fill's width to that percentage and `aria-valuenow` to the rounded percentage.
  A non-finite ratio counts as 0.

- **REQ-indeterminate-drops-valuenow** (indeterminate) —
  `setIndeterminate(true)` fills the track, animates diagonal stripes across it
  and **removes** `aria-valuenow` — a filled bar would claim a length nobody
  knows. `setIndeterminate(false)` removes the stripes; the next `set()`
  restores the value.

- **REQ-fill-eases-unless-reduced-motion** (motion) — The fill eases between
  values (`0.12s linear`), so a bar advancing a few times a second reads as
  motion rather than a stutter. Under `prefers-reduced-motion: reduce` neither
  the ease nor the stripe animation runs.

- **REQ-progress-testid-and-label** (identity) — `testId` sets the track's
  `data-testid` and `label` its `aria-label`. Callers keep their existing
  testids (`export-audio-progress`).
...
```gherkin
Scenario: A ratio sets the width and the value (REQ-determinate-announces-value)
...
Scenario: Indeterminate drops the value (REQ-indeterminate-drops-valuenow)
...
Scenario: The export dialog still reports its render through the shared bar (REQ-progress-testid-and-label)
