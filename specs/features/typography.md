...
id: typography
status: implemented
version: 2  # v2: REQ-serif-is-display-type-only carve-out — a panel-heading tab takes the heading's face
owner: ui
related:
  - architecture
  ...
  - transport-window   # the tabular-figures case (REQ-typography-rule-is-pinned-by-a-test there)
source:
...
([dropdown](dropdown.md) REQ-a-long-list-carries-a-filter) is a 10 px `<input>` the user *types into*, and
...
The rule *did* exist: as a sub-bullet of REQ-typography-rule-has-one-home in
...
This spec is that rule's real home, and its drift pin (REQ-typography-rule-is-pinned-by-a-test) is the gate CSS
otherwise never gets.

## Requirements

- **REQ-serif-is-display-type-only** — **`--serif` (Georgia) is display type
  only.** It is for text the reader **recognises at a glance**, never text they
  read for content or type into. Two families qualify:
...
- **REQ-sans-is-content-type** — **`--sans` (Inter) is content type.** Anything
  the user reads as prose or enters as data:
  ...
  - **every `<input>` and `<textarea>`** whose contents are prose or a name
    (code and pasted blobs take `--mono`, REQ-mono-is-readouts-and-pasted-text).
...
- **REQ-mono-is-readouts-and-pasted-text** — **`--mono` is readouts and pasted
  text**, and is the only face here with reliable tabular figures. Georgia ships
  proportional old-style figures with no `tnum`, so **any digit that changes in
  place** — a counter, a timer, a bar/step readout — must be `--mono`, never
  `--serif`, or it shifts sideways as it counts. This is the same finding
  [transport-window](transport-window.md) REQ-the-position-readout-is-bar-dot-step already made for the `bar.step`
  readout; it generalises.

- **REQ-text-entry-declares-no-face** — **A text-entry control never declares
  its own face.** It uses `font-family: inherit` (the `.input` pattern in
  `dialog.module.css`) or `var(--sans)` explicitly — never `var(--serif)`, and
  never nothing-at-all, since form controls do not inherit the document face by
  default. Related trap: `base.css` sets `button { font: inherit }`, so a
  *container* that gains `var(--serif)` silently re-faces every button under it.

- **REQ-typography-rule-has-one-home** — **This spec is the single home for the
  rule.** Other specs reference it rather than restating it, so there is one
  place to change when it changes. Its sibling is [iconography](iconography.md):
  a glyph that labels a control is **not type at all** and never reaches a face
  token — it is drawn. The two specs meet at the `←` keycap, which this rule
  could not save (the app bundles no font, so a face token is a request, not a
  guarantee). A spec may still record a *local* consequence (the way
  [webrtc-sync](webrtc-sync.md) notes that its body copy must not reuse
  `Modal.tagClass`).

- **REQ-typography-rule-is-pinned-by-a-test** — **The rule is pinned by
  `tests/ui/typography.test.ts`**, because nothing else can enforce it: CSS is
  exempt from `sdd-guard`, and the jsdom suite never resolves CSS Modules to
  real CSS. The pin reads the stylesheets as text and asserts that the set of
  selectors declaring `font-family: var(--serif)` **equals an explicit
  allowlist** in the test. A new serif use fails the suite until someone adds it
  to that list — which is the moment they read this spec. The list carries
  **no** standing exceptions: an entry that needs a "known deviation" note is a
  bug someone decided not to fix.
...
Scenario: every serif use is a declared one (REQ-typography-rule-is-pinned-by-a-test)
...
Scenario: a counter is never set in the display serif (regression, REQ-mono-is-readouts-and-pasted-text)
...
Scenario: the allowlist cannot rot silently (edge, REQ-typography-rule-is-pinned-by-a-test)
...
- Manual: appearance is verified **by eye** — this rule is about legibility, and
  ...
  (~198 options, so [dropdown](dropdown.md) REQ-a-long-list-carries-a-filter's filter row appears): the
  search field must read as a field, visibly distinct from the serif legends
  around it.

## Open questions / future

- The `‹ BAR n/N ›` ruler readout was the last REQ-mono-is-readouts-and-pasted-text holdout and is now `--mono`
  (see [transport-position](transport-position.md) REQ-the-readout-never-invents-bars/REQ-a-bar-stepper-only-where-bars-exist). The
...
- The tour's info badge (`tour.module.css` `.badge`) is serif *italic* and stays
  ...
  one — noted here so nobody re-files it as a REQ-mono-is-readouts-and-pasted-text bug.
