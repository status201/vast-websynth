...
id: machine-status
status: implemented
version: 4  # v4: the Arpeggiator tab gets a lamp too, driven by arp.on (REQ-the-arpeggiator-tab-has-a-lamp)
            # v3: the lane-title ↗ glyph is dropped below 720px (REQ-the-navigation-link-is-visible-at-rest)
            # v2: Chain/Mute/Solo in every machine header, shared with the Song
            #     tab's lane cards (REQ-lane-controls-live-on-both-surfaces)
owner: core
related:
  - song-mode
  - arrangement
...
- **REQ-machine-state-has-one-source-of-truth** (one source of truth) — Machine
  state derives from one pure function, `machineStatus`, which **reuses**
  `audibleLanes` (`lane-mix.ts`) rather than re-deriving the mute/solo rule.
  Every consumer (tab LED, Song panel) reads through it.
- **REQ-a-machine-has-three-states** (three states) — Each machine is `off` (its
  `<m>.on` param is 0), `muted` (enabled but inaudible — its own mute, or
  another lane's solo), or `on` (enabled and audible). Motion has no solo and
  makes no sound, so its `muted` state is driven by `motion.mute` alone
  (`params.ts` documents its active rule as `motion.on && !motion.mute`).
- **REQ-the-tab-led-is-not-a-control** (the LED is not a control) — The tab
  indicator is `pointer-events: none` with no hover, focus or cursor styling of
  its own. The tab highlights as one uniform unit; every click in the tab
  navigates. There is no code path by which the LED mutates a param.
- **REQ-machine-state-is-not-colour-only** (not colour-only) — The tab button
  carries a `title`/`aria-label` naming the machine and its state (e.g. "Drums —
  muted"), so the state survives colour-blindness and screen readers.
- **REQ-lane-titles-navigate** (titles navigate) — Each Song-panel lane title is
  a `<button>`
...
- **REQ-the-navigation-link-is-visible-at-rest** (the link is visible at rest) —
  The title carries a `↗` glyph that is legible without hover. A hover-only
  affordance is invisible on touch, and this ships as an installable PWA used on
  phones. The glyph is `aria-hidden` — the button's `aria-label` already names
  the destination, so it must not be announced as "north east arrow". (v3) **It
  is dropped below 720 px**, the app's phone breakpoint. The four lane cards are
  already two-up by 992 px, so on a phone each title shares a very narrow card
  with its Chain/Mute/Solo controls, and the glyph costs width the machine's
  *name* needs more. Nothing is lost there that the arrow was carrying: the
  affordance it exists to replace is hover, which a touch device does not have,
  and the whole card is visibly a control surface. `display: none`, not
  `visibility: hidden` — the point is to give the width back.
- **REQ-reveal-beats-activate** (reveal beats activate) —
  `TabContainer.reveal(id)` expands a collapsed tab bar *then* activates,
  matching what a real tab click does. Plain `activate()` does not expand and
  must not be the external entry point.
- **REQ-no-new-dim-behaviour** (no new dim behaviour) — The Song panel's
  existing "silenced" dimming continues to key off audibility only. A machine
  whose `<m>.on` is 0 does **not** dim its card; that state is carried by the
  LED alone.
- **REQ-lane-controls-live-on-both-surfaces** (the lane controls live on both
  surfaces, v2) — **Chain, Mute and Solo** sit in every machine header,
  immediately after that machine's on/off switch (`SEQ` / `DRUMS` / `SAMPLER` /
  `MOTION`) — the same three controls the Song tab's lane card carries, in the
  same order. The LED indicator (REQ-the-tab-led-is-not-a-control) answers *"is
  this on?"* but the fix for it was always "go to where the toggle lives", and
  for mute/solo/chain that meant the Song tab. Reading a machine's own tab and
  wanting to silence it, or to hear it in isolation while editing, meant a round
  trip. Duplicating the controls where the work happens removes it.
  - **Motion gets no Solo**, exactly as on the Song tab: it is not an audio lane,
    ...
    [motion-sequencer](motion-sequencer.md) REQ-motion-has-the-fourth-chain-lane/REQ-motion-mute-is-an-ordinary-param). Volume stays off the
...
- **REQ-the-arpeggiator-tab-has-a-lamp** (the Arpeggiator has a lamp too, v4) —
  The Arpeggiator tab carries the same indicator, driven by **`arp.on` alone**:
  `on` at ≥ 0.5, `off` below. It is the same `Tab.indicator` lamp, so
  REQ-the-tab-led-is-not-a-control (inert — every click navigates) and
  REQ-machine-state-is-not-colour-only (never colour-only — "Arpeggiator — on")
  hold unchanged. The arp is not a machine, and this requirement exists
  precisely to say so:
  ...
  - **`machineStatus` must not grow an `arp` member.** `MachineId` feeds
    ...
    REQ-machine-state-has-one-source-of-truth still holds where it means something — every tab lamp in the app is
  ...
  - **Why it earns one anyway.** With the arp engaged, key and MIDI presses stop
    ...
    the transport ([arpeggiator](arpeggiator.md) REQ-arp-takes-over-the-note-stream/REQ-a-held-key-starts-the-transport). Whether it is
...
machine-status:  # src/ui/machine-status.ts (pure core + bus adapter)
  ...
  # the arp is not a MachineId — same lamp, its own two-state adapter (REQ-the-arpeggiator-tab-has-a-lamp)
  ARP_TAB: 'arp'
  readArpStatus(bus): MachineState              # 'on' | 'off' only, never 'muted'
  subscribeArpStatus(bus, fn): () => void       # returns a disposer
  # the key/scale tab has the same shape again, over one param (REQ-the-arpeggiator-tab-has-a-lamp)
  KEY_TAB: 'key'
...
TabContainer:  # src/ui/components/tabs.ts
  ...
  # (equalizer.md REQ-the-eq-tab-led-only-indicates), which is not a machine: there `off` is bypassed,
  ...
  reveal(id): void                              # expand-then-activate (REQ-reveal-beats-activate)
...
arp (REQ-the-arpeggiator-tab-has-a-lamp, outside the rule above):
...
reaches the middle brightness (REQ-the-arpeggiator-tab-has-a-lamp) — no new CSS, no arp-specific look. A lamp
...
Scenario: Solo elsewhere mutes an enabled machine (REQ-machine-state-has-one-source-of-truth)
...
Scenario: A lane title opens its machine tab (REQ-lane-titles-navigate)
...
Scenario: reveal expands a collapsed tab bar (REQ-reveal-beats-activate)
...
Scenario: The LED tracks the bus without being clickable (REQ-the-tab-led-is-not-a-control)
...
Scenario: Every machine header carries Chain, Mute and Solo (v2, REQ-lane-controls-live-on-both-surfaces)
...
Scenario: Motion's header has no Solo (edge, v2, REQ-lane-controls-live-on-both-surfaces)
...
Scenario: The header controls and the Song tab stay in lock-step (v2, REQ-lane-controls-live-on-both-surfaces)
...
Scenario: The Arpeggiator lamp follows arp.on (v4, REQ-the-arpeggiator-tab-has-a-lamp)
...
Scenario: The Arpeggiator is never 'muted' (edge, v4, REQ-the-arpeggiator-tab-has-a-lamp)
...
Scenario: Song is the only tab without a lamp (v4, REQ-the-arpeggiator-tab-has-a-lamp)
  Given the app is booted
...
- The Song panel's lane cards could carry the same LED next to their titles. Left
  out for now: the cards already dim when silenced (REQ-no-new-dim-behaviour), so a second signal in
