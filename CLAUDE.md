...
in CI. `npm run spec:lint` checks spec *structure* — and that every `x.md REQ-n`
citation (in code comments too) and every backticked code name in a spec still
resolves — and also runs as a `Stop` hook.
