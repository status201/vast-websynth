...
`*.md`, `*.css`, any path under a `styles/` dir, `src/vendor/**`, `src/state/demos/**`
(demo songs are data drop-ins) and the two demo *metadata* files that ride with
them — `src/state/demo-notes.json` (hand-written blurbs) and
`src/state/demos-index.json` (generated by `clean:demos`). For a genuinely trivial
*production* tweak:
`touch .sdd-skip` locally, or `[skip-sdd]` in a commit / the `skip-sdd` PR label
in CI. `npm run spec:lint` checks spec *structure* — that a **new REQ id is a
slug** and not a number (ADR-021), that every `x.md REQ-<id>` citation (in code
comments too) and every backticked code name in a spec still resolves — and also
runs as a `Stop` hook.
...
- `npm run typecheck` is the primary gate (strict + `noUncheckedIndexedAccess`, so
  expect `arr[i]!` assertions throughout — match that style; plus `noUnusedLocals`
  / `noUnusedParameters`, so dead code fails the gate — prefix a deliberately
...
- **UI and audio never call each other.** A control writes `bus.set(...)`; the
  Engine `bus.subscribe(...)`s. → `specs/architecture.md` REQ-ui-and-audio-never-call-each-other, ADR-001.
